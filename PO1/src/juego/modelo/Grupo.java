...
public class Grupo {
	...
	public boolean contiene(Celda celda) {
	    for (Celda celdaComparar: celdas)
	        if (celda.obtenerFila() == celdaComparar.obtenerFila() &&
                    celda.obtenerColumna() == celdaComparar.obtenerColumna())
	            return true;
	    return false;
	...
	public Grupo generarCopiaEnOtroTablero(Tablero otroTablero) {
        for (Celda celda : celdas)
            otroTablero.colocar(celda.obtenerPiedra(), otroTablero.obtenerCeldaConMismasCoordenadas(celda));
        Grupo nuevo = new Grupo(otroTablero.obtenerCeldaConMismasCoordenadas(celdas.get(0)), otroTablero);
        for (int i = 1; i < obtenerTamaño(); i++) {
            nuevo.añadirCeldas(new Grupo(otroTablero.obtenerCeldaConMismasCoordenadas(celdas.get(i)), otroTablero));
        }
        return nuevo;
	}
